...
## ADR-026 — The export list comes from the SERVER; a browser file input could never have worked

**The bug.** ADR-025 shipped the sidecar behind a native `<input type="file">`. A file input opens the *client's* filesystem, so clicking it showed the user their own Mac's Downloads while the exports sat on the bench at `sites/metabase_sql`. That is a browser security boundary, not a path or a permission to configure — the control could not work for any user, on any OS, however they navigated. It was found by one click on the real page.

**Why the tests all passed.** Every check was on the client half: files were fed to the input with Playwright's `setInputFiles`, and the pairing, the JSON parse and the refusals were all correct. Nothing asked *where the files a real user could reach actually are*, and the fixture answered a question the deployed page never gets to ask. The lesson is narrower than "test more": a fixture that supplies the input under test cannot discover that the input has no supply.

**The fix.** Two read-only endpoints in `api/exports.py` — `list_exported_reports(search)` returns names and whether each has a sidecar; `read_exported_report(stem)` returns that one report's SQL and parsed sidecar. The UI is a search box, so typing "QIPI" finds "Quality & Innovation Performance Index (QIPI)". That also settles the original complaint — browsing ~1775 files by raw filename — as a side effect, because it was the same root cause.

**The traversal guard is structural, and it has to be.** This reads server files by a name a browser sent, and `site_config.json` — holding `metabase_api_key` — is one directory above the folder. So a requested name is only ever **compared against the folder's own listing**, never joined onto it: there is no string a caller can send that becomes a different path. `../../site1/site_config` matches no entry. A second check, `_inside`, resolves each entry and requires its real parent to still be the export folder, which closes the other half — a `.sql` *symlink* planted in the folder would otherwise be listed and read like any export. Only someone with write access to the folder could plant one, but "you would need access first" is the argument every traversal bug makes, and the check is three lines. The reader's `read_exported_report` is asserted to contain **no path join at all**, by AST rather than by grep: the first version of that test searched the source text for `folder /` and matched the module docstring explaining why there is no `folder /`.
...
## ADR-033 — Card 2076's real blocker was one invisible space, and the fix is Insights' own rename

**The reconstruction converted; the real card refused after a full deploy of ADR-032.** The verbatim capture (now `dashboard_studio/tests/fixtures/employee_satisfaction_2076.sql`) showed why: Metabase's compiled alias is `` `Exit  Qn. 7` `` with a DOUBLE space. The lift whitespace-normalised each item before reading its alias, so the renames map held the single-space spelling while the outer SELECT still referenced the double-space one — an unmapped reference, a decline, and a refusal saying "subquery" about a character nobody can see. The alias is now read from the item's RAW text; normalisation still applies to everything else in the item. A reconstruction cannot carry a detail its description omitted — which is why the capture rule exists, and why this fixture is checked in verbatim.

**The second blocker was latent behind the first: those aliases reach expression text.** The ADR-011 mutate would read `avg_of_Exit  Qn. 7` — and Insights evaluates expressions with `ast.parse` (ADR-032), where a name with a space or dot is a SyntaxError on open. A type or a quoting scheme cannot fix this; the name itself has to be writable. **Every computed alias the lift creates is therefore slugged with `_insights_name` — a verbatim port of Insights' own `sanitize_name` (`ibis_utils.py` v3.12.2)** — before it can reach any operation. The safety argument is read from Insights' source, not asserted: `apply_mutate` runs every created column's name through `sanitize_name` itself, so the engine column was ALWAYS going to be `exit__qn__7` whatever new_name we stored, and `get_column`'s sanitized-name fallback is what made raw spellings appear to work. Slugging changes no name a person sees in the query's output; it makes the stored JSON agree with the engine byte for byte. Two aliases reducing to one slug refuse by both names — merging two columns silently would answer a different question.

**What is NOT slugged, on purpose: chosen labels.** The outer items — `` `Staff Onboarding Average (Obj. 5)` `` — are names a person typed into Metabase and reads back in Insights. Nothing downstream evaluates them as text (nothing reads an ADR-011 result), so they keep their raw spelling, and Insights sanitizes them on open exactly as it always did. The line is: a name that reaches expression text must be writable in Python and is slugged; a name that is only ever a JSON `new_name` is a label and is left alone.

**ADR-032's digit guard generalised to "must be a valid Python identifier".** `isidentifier()` plus a keyword check, at the same choke point — a digit-led name, a spaced name, a dotted name and `class` all fail the same way inside `ast.parse`, so one rule covers them and the refusal names the column. The digit spelling of the message is gone; `bulk_dry_run.py`'s group table follows it.

**A slug named `year` found a scanner bug that predates it.** `_alias_referenced_outside` scanned mutate expressions with a bare word-boundary match, so a mutate slugged to `year` "matched" the `year(...)` CALL inside every other date expression, was counted as referenced, and dropped out of the promotion candidates — leaving its twin on the same column a "singleton" that promoted, recreating exactly the double grouping the ADR-030 collision guard refuses. A name followed by `(` is a call, never a column: the scan now carries `(?!\s*\()`. The regrouped-month fixture pins it.

Fixture-tested end to end; the live re-run of `wrapper_residue.py` on card 2076 after a deploy is the user's step.

## ADR-034 — A refused proposal is handed back to the model for a bounded, visible correction

**The observed miss:** the model emitted `ORDER BY 'count'` when its own query produces `count_of_name`. The converter refused — correctly — and a person was left reading a dialect fault and rewording a question that was fine. The refusal text already says exactly what to fix, so it goes back to the model with the refused SQL, and the corrected reply runs through the whole pipeline again. **No new trust**: every retry is an ordinary `propose_from_question` call — same validation, same allowlists, same widening check, same DocType confirm upstream (untouched; retry cannot address the table-choice risk and does not try). **No new egress**: the previous SQL is text the model itself wrote, and the refusal is composed by our converter from that SQL and a schema already in the same request.

**The loop lives in the PAGE, and that is what makes it bounded and visible.** Each retry is one request; `state.retrying` re-labels the build button ("Refused — correcting the SQL (attempt 2 of 3)…") through the ADR-028 busy machinery, so a longer wait is never a silent one. Two corrected attempts maximum; exhaustion shows the LAST refusal, which is the one the model could not get past. The server's whole part is two optional keywords (`previous_sql`, `refusal`) reaching `write_sql_request` — asserted by AST to reach the one prompt builder and nothing else.

**Which refusals retry — `retryableRefusal` in `studio_core.js`, and the line is written down:** a fault in how the SQL was WRITTEN (an alias the query does not produce, a bare column, an invented name, `IN` where OR-ed equalities say the same thing, `BETWEEN` where `>= AND <=` does) is fixable and retries; a fault in what the question NEEDS — a concept the Insights operation vocabulary lacks — can never convert however it is respelled, and retrying only burns tokens. Structural, by the converter's own message spellings: mixed AND+OR, CASE (both spellings), UNION, HAVING, DISTINCT, window functions, subqueries/EXISTS, IS NULL, REGEXP, NOT, CROSS join, the same table twice, and LIKE (both its spellings — there is no pattern operator, and `=` answers a different question). The bare one-word markers ("UNION", "DISTINCT") match whole reasons only, so a DocType named "Union Bank" inside another message cannot trip them. One structural reason blocks the retry even beside fixable ones. A message on neither list retries: bounded either way, and an unknown message is far more likely a dialect fault than a new concept. A `CANNOT` from the model and an empty reply do not retry — there is no SQL to correct.

**The prompt now states the produced-columns rule for BOTH clauses**: every ORDER BY and every GROUP BY must name a column the SELECT list produces, spelled exactly — the second half of the fix, so the common case never needs the retry at all.

## ADR-035 — Reconciling NUMBERS, and why it does not call Metabase

689 reports convert; exactly one has had its figures checked, by a person, by eye. Every automated check in this repo proves a query CONVERTS and OPENS. `scripts/reconcile_numbers.py` runs both halves of a card and compares the rows, with the comparison itself in `integrations/reconcile.py` — Frappe-free and unit-tested, like the parser, because it is the part with the subtle mistakes in it.

**It does not call Metabase, and does not need the endpoint that would.** Running a card requires `POST /api/card/:id/query` (or its `/csv`/`/json` export twins, or `POST /api/dataset`) — every one of them executes against the production database, and they are on this project's never-add list. They are also unnecessary: the exported `.sql` beside each card IS the SQL Metabase runs — native cards verbatim, GUI cards compiled by Metabase itself through the already-approved compile-only endpoint — and Metabase reads the same database the bench does. Running that SQL here is the same comparison with a smaller blast radius, and a **better** one: it isolates our translation from Metabase's rendering, which is the only thing in question.

**What it executes and how that is bounded.** One statement, checked by `_read_only` to be a single SELECT before the database sees it, wrapped in a row cap, rolled back after. The export folder is not a trusted input — anyone who can reach the bench can write there — so the check is on the text, not on the situation. String literals and backticked identifiers are stripped before the scan, on ADR-032's own argument: a backticked name is an identifier, so a column called `` `update` `` is not an UPDATE and a value of `'x; y'` is not a second statement. Both appear in the real corpus. The durable control is unchanged: a SELECT-only database login.

**What a match proves, and — recorded because it contradicts the brief — what it cannot.** It proves our operations compute what the card's SQL computes. It cannot prove the CARD is right. **Join fan-out and an unfiltered `docstatus` are card-fidelity faults, not translation faults**: if the card fans out a one-to-many join, we reproduce it exactly, both sides agree, and the harness correctly reports a match over two identically wrong numbers. Only a fan-out our translation INTRODUCES is caught. What it does catch: MySQL-versus-ibis semantics (integer division, rounding, string case, `COUNT(col)` against NULLs), a join whose type changed in translation, a filter that did not survive, a grouping that collapsed, and slug collisions. Finding the card's own fan-out needs a different instrument — comparing a query's row count against its source table's — and that is not this.

**Two tolerances, because one cannot do it.** MySQL's `AVG()` over exact values returns DECIMAL at the column's scale PLUS FOUR, so the card says 3.3333 where ibis says 3.3333333333333335 — a formatting difference landing at ~1e-5 relative, which a useful relative tolerance would flag on every average in the corpus. `ABS_TOL = 1e-4` absorbs exactly that; `REL_TOL = 1e-6` covers large values. Both sit orders of magnitude below any real fault — a fan-out doubles, a swapped column changes magnitude, integer division truncates. **The report carries the largest delta per column even when it passes**, so the tolerance argues with itself: a corpus landing at 1e-5 confirms the reasoning, one landing at 1e-2 is a finding wearing a pass.

**Two things the mutation pass found, both real.** Rows are sorted before comparison (a query with no ORDER BY may return them in any order), and sorting numbers BY THEIR TEXT is consistent right up until it is not: 10.0 keys as `"10.0"` and its float64 twin 9.9999999 as `"9.99…"`, landing the other side of a row worth 9.5 — the two sides sort differently and every row is then compared against the wrong one. Numbers now sort as numbers, rounded to 6 significant figures. Separately, the single-statement check was redundant against the verb scan for every case tested; two SELECTs now pin it, so the guard does not rest entirely on a list of words.

**Leftover columns are never paired by position.** The card's `AVG(x) AS avg` is our `avg_of_x` and they do not match by name, so they are reported unmatched and the run prints a ready-to-fill `column_map`. A positional pairing can agree as easily as it can disagree, and a false agreement here is worse than no answer — this is the one place a guess would undermine the only correctness instrument the project has.

## ADR-036 — The DocType shortlist is ranked from the schema, not sampled from a model

**The shortlist was non-deterministic, and it is the one step that cannot be.** The same question returned a different set of DocTypes on each run. ADR-023 records that confirm step as the entire defence against the wrong-table failure — a question about recruitment agents answered from ERPNext's sales-commission tables, every column real, every type right, nothing for a referential validator to object to. A defence that varies run to run is a coin toss the person cannot audit.

**Diagnosed before anything was built, and the request turned out to be innocent.** The payload is byte-identical between runs: `pick_doctypes_request` sorted the DocType names, so `frappe.get_all`'s default `modified desc` ordering never reached the prompt, and `frappe.get_all` overrides `limit_page_length` to 0 (read from `frappe/__init__.py`, whose own docstring says "Default 20" two lines above the code that sets 0) so the list was never truncated either. Model fixed, token cap fixed, no nonce. **The variance was entirely provider-side sampling.**

**No sampling setting could have fixed it, and this is from the SDK rather than recalled.** We send no `temperature`, so the API default applies. openai-python 2.53.0 documents `temperature` as an ordinary chat parameter and says nothing about any model rejecting it — so the "newer models reject temperature" claim was NOT confirmed and is not relied on. What settles it is the neighbouring parameter: `seed` documents itself as *"in Beta… best effort… Determinism is not guaranteed."* The provider declines to promise determinism at all. A `temperature=0` stop-gap was offered and refused on exactly the right grounds — a fix that looks like a fix but is not is worse than the visible problem.

**So the LLM is gone from this step.** `integrations/doctype_rank.py` ranks candidates from facts already on the site: the DocType's name, its module, and the labels and fieldnames of its DocFields. Frappe-free and injected, so it is unit-testable without a bench; `api/propose.py` reads the catalogue and calls it. Determinism is structural — ties break by NAME, never by catalogue order, because `frappe.get_all` returns DocTypes `modified desc` and letting that reach the result would recreate the same drift wearing different clothes.

**The evidence is the bigger win, and it is not the rationale ADR-023 refused.** Each candidate carries what it matched on — "the field labelled 'Agent Name' matched 'agent'". A model's rationale describes an intention while the query does something else, which is why returning one was refused; a match on a real field label is the fact itself, and a person can disagree with it. The confirm step stops being a list to rubber-stamp.

**Two side effects worth naming.** Nothing about a question leaves the site any more until the tables are settled — the old first pass sent the question plus *every DocType name on the site* to the provider — and choosing tables no longer needs an API key at all. The LLM picker was deleted rather than left beside the ranker, so there is no sampled path left to call by accident.

**What this cannot do:** synonyms. "Staff" will not find `Employee` by itself. That is the honest cost, blunted by ranking over field labels (an Employee DocType usually has a field labelled "Staff Number"), by offering more candidates than the model's four, and by the manual-entry path. **Phase 2 — an LLM re-ranker constrained to reorder the factual list, never to add to it — is deliberately NOT built**: whether it is needed is a question to answer by measuring this against real questions, and building it first would be paying for a component nothing has shown to be necessary.
