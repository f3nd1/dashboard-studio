...
function renderValueBars(node,rows){const max=chartMax(rows);node.innerHTML=`<div class="ucc-demo-bars">${rows.map(function(row){return`<div class="ucc-demo-bar"><label>${esc(row[0])}</label><div><i style="width:${Math.max(4,finiteNumber(row[1],0)/max*100)}%"></i></div><strong>${row[1]}</strong></div>`;}).join("")}</div>`;}
...
registerChartPlugin("value-bars",(node,rows)=>renderValueBars(node,rows));
...
function customDatasetRows(result,chart){
if(!chart||!chart.dataKey)return[];
let value=result;
String(chart.dataKey).split(".").forEach(function(part){if(value!=null)value=value[part];});
if(!Array.isArray(value))return[];
return value.map(function(item){
if(Array.isArray(item))return[item[0],finiteNumber(item[1],0),null,true,item];
const label=item&&item.label!=null?item.label:(item&&item.year!=null?item.year:(item&&item.metric!=null?item.metric:""));
const number=finiteNumber(item&&item.value,NaN);
return[label,number,null,true,item];
}).filter(function(row){return row[0]!==""&&Number.isFinite(row[1]);});
}
function metricRows(result,chartIndex,chart){
const customRows=customDatasetRows(result,chart);
