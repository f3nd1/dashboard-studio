...
EMPLOYEE_SATISFACTION_PARENT = "Employee Satisfaction Index"
EMPLOYEE_SATISFACTION_YEAR_CHILD = "Quality Performance Outcomes Performance Childtable"
EMPLOYEE_SATISFACTION_METRIC_CHILD = "Quality Performance Actual Value Parameter Childtable"
...
def table_field_for_child(parent_doctype, child_doctype):
    meta = get_meta(parent_doctype)
    if not meta:
        return ""
    try:
        for meta_field in meta.fields or []:
            if meta_field.fieldtype == "Table" and meta_field.options == child_doctype:
                return meta_field.fieldname
    except Exception:
        pass
    return ""


def sort_year_values(values):
    numeric = []
    text_values = []
    ...
    output.extend(text_values)
    return output


def fetch_employee_satisfaction_visuals():
    output = {
        "status": "unavailable",
        "message": "",
        "parent_doctype": "Quality Performance Outcomes",
        "parent_name": EMPLOYEE_SATISFACTION_PARENT,
        "per_year": [],
        "per_metric": [],
        "latest": None,
        "source_mapping": [],
    }
    source = resolved_sources.get("quality_performance") or {}
    parent_doctype = source.get("doctype")
    if source.get("status") != "available" or not parent_doctype:
        output["status"] = source.get("status") or "unavailable"
        output["message"] = source.get("message") or "Quality Performance Outcomes is unavailable."
        return output

    parent_fields = safe_fields(
        parent_doctype,
        ["name", "date_of_review", "outcome_indicator", "outcomes_category", "benchmark_target", "rating_type", "max_value", "min_value", "status"],
    )
    try:
        readable_parent = frappe.get_list(
            parent_doctype,
            fields=parent_fields,
            filters={"name": EMPLOYEE_SATISFACTION_PARENT},
            limit_page_length=1,
        ) or []
    except Exception as error:
        output["status"] = "permission_denied" if is_permission_error(error) else "query_error"
        output["message"] = clean_text(error)
        return output
    if not readable_parent:
        output["status"] = "unavailable"
        output["message"] = "The Employee Satisfaction Index record is not readable or does not exist."
        return output

    try:
        parent_doc = frappe.get_doc(parent_doctype, EMPLOYEE_SATISFACTION_PARENT)
    except Exception as error:
        output["status"] = "permission_denied" if is_permission_error(error) else "query_error"
        output["message"] = clean_text(error)
        return output

    requested_year = clean_text(filters.get("review_year") or filters.get("year") or filters.get("monitoring_year"))
    year_table_field = table_field_for_child(parent_doctype, EMPLOYEE_SATISFACTION_YEAR_CHILD)
    metric_table_field = table_field_for_child(parent_doctype, EMPLOYEE_SATISFACTION_METRIC_CHILD)
    output["source_mapping"] = [
        {
            "dataset": "per_year",
            "child_doctype": EMPLOYEE_SATISFACTION_YEAR_CHILD,
            "parentfield": year_table_field,
            "fields": ["year", "value"],
        },
        {
            "dataset": "per_metric",
            "child_doctype": EMPLOYEE_SATISFACTION_METRIC_CHILD,
            "parentfield": metric_table_field,
    ...
    year_groups = {}
    if year_table_field:
        for row in parent_doc.get(year_table_field) or []:
            ...
            if year not in year_groups:
                year_groups[year] = {"sum": 0.0, "count": 0}
            year_groups[year]["sum"] = year_groups[year]["sum"] + value
            year_groups[year]["count"] = year_groups[year]["count"] + 1
    else:
        warnings.append("No parent Table field resolves to " + EMPLOYEE_SATISFACTION_YEAR_CHILD + ".")

    for year in sort_year_values(list(year_groups.keys())):
        group = year_groups.get(year) or {}
        count = group.get("count") or 0
        if count:
            value = round((group.get("sum") or 0.0) / count, 2)
            output["per_year"].append({
                "label": year,
                "year": year,
                "value": value,
                "record_count": count,
    ...
    metric_groups = {}
    if metric_table_field:
        for row in parent_doc.get(metric_table_field) or []:
            metric = clean_text(row.get("metric"))
            year = clean_text(row.get("year"))
            value = to_number(row.get("actual_value"))
            if not metric or not year or value is None:
                continue
            if requested_year and year != requested_year:
                continue
            group_key = metric + "|||" + year
            if group_key not in metric_groups:
                metric_groups[group_key] = {"metric": metric, "year": year, "sum": 0.0, "count": 0}
            metric_groups[group_key]["sum"] = metric_groups[group_key]["sum"] + value
            metric_groups[group_key]["count"] = metric_groups[group_key]["count"] + 1
    else:
        warnings.append("No parent Table field resolves to " + EMPLOYEE_SATISFACTION_METRIC_CHILD + ".")

    metric_rows = []
    for group_key in metric_groups:
        group = metric_groups.get(group_key) or {}
        count = group.get("count") or 0
        if count:
            metric_rows.append({
                "label": clean_text(group.get("metric")) + " · " + clean_text(group.get("year")),
                "metric": group.get("metric"),
                "year": group.get("year"),
                "value": round((group.get("sum") or 0.0) / count, 2),
                "record_count": count,
            })
    def metric_row_sort_key(item):
        return (clean_text(item.get("metric")), clean_text(item.get("year")))
    metric_rows.sort(key=metric_row_sort_key)
    output["per_metric"] = metric_rows

    if output["per_year"]:
        output["latest"] = output["per_year"][-1]
    if output["per_year"] or output["per_metric"]:
        output["status"] = "available" if not warnings else "partial"
        output["message"] = " ".join(warnings)
    else:
        output["status"] = "partial" if warnings else "available"
        output["message"] = "No numeric Employee Satisfaction Index child rows match the current filter."
    return output
...
employee_satisfaction = fetch_employee_satisfaction_visuals()
latest_employee_satisfaction = employee_satisfaction.get("latest") or {}
metrics.append({
    "id": "c711-employee-satisfaction-latest",
    "label": "Latest Employee Satisfaction Index",
    "source": "quality_performance",
    "doctype": (resolved_sources.get("quality_performance") or {}).get("doctype"),
    "display_doctype": display_doctype((resolved_sources.get("quality_performance") or {}).get("doctype")),
    "value": latest_employee_satisfaction.get("value"),
    "record_count": latest_employee_satisfaction.get("record_count") or 0,
    "total": latest_employee_satisfaction.get("record_count") or 0,
    "denominator": None,
    "unit": "index",
    "status": employee_satisfaction.get("status") if latest_employee_satisfaction else "unavailable",
    "message": employee_satisfaction.get("message") or "",
    "resolved_fields": ["year", "value"],
...
metrics.append({
    "id": "c711-employee-satisfaction-metric-observations",
    "label": "Employee Satisfaction metric-year observations",
    "source": "quality_performance",
    "doctype": (resolved_sources.get("quality_performance") or {}).get("doctype"),
    "display_doctype": display_doctype((resolved_sources.get("quality_performance") or {}).get("doctype")),
    "value": len(employee_satisfaction.get("per_metric") or []),
    "record_count": len(employee_satisfaction.get("per_metric") or []),
    "total": len(employee_satisfaction.get("per_metric") or []),
    "denominator": None,
    "unit": "observations",
    "status": employee_satisfaction.get("status"),
    "message": employee_satisfaction.get("message") or "",
    "resolved_fields": ["metric", "year", "actual_value"],
    "resolved_field_groups": [],
    "rows": [],
    "truncated": False,
    "evidence_level": "stored_calculation",
...
result = {
    ...
    "warnings": [
        ...
        "Employee Satisfaction Index visuals use the exact parent record and child DocTypes supplied by the Metabase models. Duplicate year or metric-year rows are averaged and disclosed through record_count.",
