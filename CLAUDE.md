...
**An ORDER BY is dropped, on purpose, and the proof is the row limit next door.** It is read only as a clause boundary; three of the four checked-in captures carry one that goes nowhere. Unlike the dropped LIMIT and the dropped computed column, this one changes no row and no value: **ORDER BY selects which rows only alongside a LIMIT, and a real LIMIT refuses**. It still costs the reading order of a chart, so it is a trade — pinned by `TestOrderByIsDroppedOnPurpose`, and settled properly only by finding out whether Insights stores an order operation.
