...
**`col * 1` on a TEXT column is allowed as an aggregate argument, and it is a defect allowance, not a feature — read ADR-009 before touching it.** Metabase writes `* 1` to cast before aggregating; at UCC it does that to `actual_value`, a Frappe **Data** field, so the live report averages text by silent coercion. Refusing blocked the report, so it converts — and records `coerced_from: "String"` on the measure, which the operations list renders as *"String coerced to a number"*, because every non-numeric row is averaged in as 0 and nothing else would show that. Still refused in a GROUP BY. The proper fix is retyping the Frappe field; if that happens, this rule should go rather than linger.
