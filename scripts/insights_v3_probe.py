...
Paste the whole file into::
    ...
    bench --site ucc.local console

or pipe it in: ``bench --site ucc.local console < scripts/insights_v3_probe.py``.
...
ponytail: no blank lines inside any function body, on purpose. IPython reading
from stdin ends an indented block at the first blank line, and a diagnostic that
only works when pasted one way is a diagnostic that wastes a round trip.
"""
...
def exists(doctype):
    return bool(frappe.db.exists("DocType", doctype))  # noqa: F821
...
def versions():
    apps = frappe.get_installed_apps()  # noqa: F821
    print("  installed apps:", ", ".join(apps))
    for app in ("frappe", "insights", "dashboard_studio"):
        if app not in apps:
            continue
        try:
            print(f"  {app} version:", frappe.get_attr(f"{app}.__version__"))  # noqa: F821
...
def doctypes_and_counts():
    ...
    for doctype in V2_DOCTYPES + V3_DOCTYPES:
        if not exists(doctype):
            ...
            continue
        try:
            count = frappe.db.count(doctype)  # noqa: F821
        except Exception as exc:  # noqa: BLE001
    ...
    if v2 and v3:
        print("  -> BOTH generations ship together, which is exactly why the current")
        print("     version guard (exists('DocType', 'Insights Query')) passes on v3.")
...
def fields(doctype):
    ...
    if not exists(doctype):
        ...
        return
    meta = frappe.get_meta(doctype)  # noqa: F821
    ...
    for field in meta.fields:
        ...
        if field.unique:
            flags.append("unique")
        options = f" -> {field.options}" if field.fieldtype in ("Link", "Table") else ""
        suffix = ("  [" + ", ".join(flags) + "]") if flags else ""
        print(f"    {field.fieldname:26} {field.fieldtype:12}{options}{suffix}")
...
def data_sources():
    ...
    for doctype in ("Insights Data Source", "Insights Data Source v3"):
        if not exists(doctype):
            print(f"  {doctype}: absent")
            continue
        rows = frappe.get_all(doctype, fields=["name"], limit=50)  # noqa: F821
...
def v3_results():
    ...
    others = frappe.get_all("DocType", filters={"name": ["like", "%Insights%"]},  # noqa: F821
    ...
    total = frappe.db.count("Insights Query Result")  # noqa: F821
    print(f"  Insights Query Result rows: {total}")
    if not exists("Insights Query v3"):
        return f"{total} rows, no v3 to compare"
    v3_names = {r["name"] for r in frappe.get_all("Insights Query v3",  # noqa: F821
                                                  fields=["name"], limit=1000)}
    rows = frappe.get_all("Insights Query Result",  # noqa: F821
    ...
    if hits:
        ...
        print("  -> none. That is the expected v3 answer: results are computed on")
        print("     demand and cached briefly, not stored. apply_insights_chart's")
        print("     'read the executed columns' step has no v3 counterpart.")
...
def v3_chart_config():
    ...
    rows = frappe.get_all(  # noqa: F821
        "Insights Chart v3",
        fields=["name", "title", "chart_type", "query", "workbook", "config", "modified"],
        order_by="modified desc", limit=200,
    )
    ...
    if not configured:
        print("\n  NOTHING TO READ. Build one by hand, then re-run this script:")
        print("    Insights -> the workbook holding the query above -> add a chart ->")
        print("    pick Bar -> set an X axis and a Y axis -> save.")
        if rows:
...
def studio_records():
    ...
    print("  migrates them, so they are expected to be intact and invisible to the")
    print("  v3 UI rather than converted.")
...
print("# SUMMARY")
print("#" * 78)
for _label, _note in REPORT:
    print(f"  {_label:58} {_note}")
print("""
